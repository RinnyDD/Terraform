...
resource "aws_security_group" "asg" {
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["119.13.57.231/32"]  # REPLACE WITH YOUR IP!
  }
...
resource "aws_security_group" "db" {
  ...
  ingress {
    from_port       = 3306  # MySQL default port (change for PostgreSQL/Oracle)
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [aws_security_group.asg.id]  # Only ASG can access DB
  }
